...
public final class AppCompatibility {
    private static final String AIYIFAN_PACKAGE = "tv.ifvod.classic";
    private static final String YOUTUBE_KIDS_PACKAGE =
    ...
    public static int statusResource(
        ...
        if (installed && YOUTUBE_KIDS_PACKAGE.equals(app.packageName)) {
            return R.string.compat_youtube_kids_shared_runtime;
        }
        if (!profile.packageInstaller
                && !hasUsableStore(profile)
                && !OfficialWebRuntime.isAvailable(app)) {
        ...
        if (GOOGLE_NATIVE_PACKAGES.contains(app.packageName)) {
            ...
            return OfficialWebRuntime.isAvailable(app)
                    ? R.string.compat_google_library_web
                    : R.string.compat_google_native_blocked;
        }
        if (GOOGLE_TV_PACKAGES.contains(app.packageName)) {
            ...
            return OfficialWebRuntime.isAvailable(app)
                    ? R.string.compat_google_web_available
                    : R.string.compat_platform_variant;
        }
    ...
    public static boolean prefersOfficialWeb(
            CatalogApp app,
            DeviceProfile profile
    ) {
        if (!OfficialWebRuntime.isAvailable(app)) {
            return false;
        }
        if (app.source == CatalogApp.Source.CHINA_TV_STORE
                && !profile.xiaomiStore) {
            return true;
        }
        if (AIYIFAN_PACKAGE.equals(app.packageName)) {
            return true;
        }
        if (PLAY_CERTIFIED_VIDEO_PACKAGES.contains(app.packageName)
                && (!profile.isGoogleReady() || !profile.hasHardwareWidevine())) {
            return true;
        }
        return !profile.isGoogleReady()
                && (GOOGLE_NATIVE_PACKAGES.contains(app.packageName)
                || GOOGLE_TV_PACKAGES.contains(app.packageName)
                || PLAY_CERTIFIED_VIDEO_PACKAGES.contains(app.packageName));
    }
