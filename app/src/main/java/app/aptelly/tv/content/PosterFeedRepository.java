...
public final class PosterFeedRepository {
    ...
    private static final String KEY_REFRESH = "last_refresh";
    ...
    public void start(FeedListener listener) {
        ...
        if (TextUtils.isEmpty(BuildConfig.POSTER_FEED_URL)) {
            return;
        }
        boolean sourceChanged = !BuildConfig.POSTER_FEED_URL.equals(
                preferences.getString(KEY_SOURCE_URL, "")
        );
        long age = System.currentTimeMillis() - preferences.getLong(KEY_REFRESH, 0L);
        if (!sourceChanged && age < REFRESH_INTERVAL_MS) {
            return;
        }
        executor.execute(() -> {
            try {
                String json = downloadText(BuildConfig.POSTER_FEED_URL);
                Map<String, PosterScene> parsed = parse(json);
                if (parsed.isEmpty()) {
                    return;
                }
                remoteScenes = parsed;
                preferences.edit()
                        ...
                        .putLong(KEY_REFRESH, System.currentTimeMillis())
                        .putString(KEY_SOURCE_URL, BuildConfig.POSTER_FEED_URL)
                        .apply();
                if (!closed) {
                    mainHandler.post(listener::onFeedChanged);
                }
            } catch (Exception ignored) {
                // Cached and generated scenes remain available while the feed is offline.
            }
        });
    }
        ...
        try {
            ...
            for (int index = 0; index < groups.length(); index++) {
                ...
                result.put(category, new PosterScene(
                        category,
                        item.optString("eyebrow", "APTELLY · TRENDING"),
    ...
    private String downloadText(String address) throws Exception {
        return new String(
                downloadBytes(address, MAX_FEED_BYTES),
