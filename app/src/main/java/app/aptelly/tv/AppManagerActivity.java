...
import app.aptelly.tv.catalog.ProductVariantPolicy;
...
public final class AppManagerActivity extends Activity {
    ...
    private void rebuild() {
        ...
        addGroup(getString(R.string.popular_apps), AppCatalog.popular(), true);
        addGroup(
                getString(R.string.china_mainland_apps),
                AppCatalog.chinaMainlandServices(),
                true
        );
        addGroup(getString(R.string.global_apps), AppCatalog.globalServices(), true);
        addGroup(getString(R.string.regional_apps), AppCatalog.regionalServices(), true);
    ...
    private void addGroup(
        ...
        if (!keepUnavailable) {
            ...
            for (CatalogApp app : apps) {
                if (CatalogAvailability.isVisible(app.packageName)
                        || InstalledAppResolver.installedPackage(
                                this,
                                app.packageName
                        ) != null) {
                    visible.add(app);
    ...
    private View buildAppRow(CatalogApp app) {
        ...
        String installedPackage = resolution.status == ProductVariantPolicy.Status.READY
        ...
        String state = installed
                ...
                : resolution.status == ProductVariantPolicy.Status.UPDATE_REQUIRED
                        ? getString(R.string.installed_update_required)
                : resolution.status == ProductVariantPolicy.Status.MIGRATION_REQUIRED
                        ? getString(R.string.installed_migration_required)
                : resolution.status == ProductVariantPolicy.Status.RUNTIME_REQUIRED
                        ? getString(R.string.installed_runtime_required)
                : unavailable
    ...
    private void checkForUpdate(CatalogApp app) {
        ...
        updateExecutor.execute(() -> {
            try {
                ...
                NetworkStatus network = NetworkPreflight.inspect(this);
                String message;
                if (upToDate) {
