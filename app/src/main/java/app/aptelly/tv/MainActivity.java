...
public final class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hideSystemBars();
        updateClock();
        posterRepository.start(() -> updateBackdrop(category(selectedCategory)));
    ...
    private View buildSidebar() {
        ...
        TextView mark = text("TV", 13, Color.WHITE, true);
        mark.setGravity(Gravity.CENTER);
        mark.setLetterSpacing(0.06f);
        mark.setBackground(brandMarkBackground());
    ...
    private void renderCategory(String key, boolean requestContentFocus) {
        ...
        List<LauncherActivityInfo> installedApps = current.kind == CategoryKind.INSTALLED
                ? launchableApps()
                : new ArrayList<>();
        if (current.kind == CategoryKind.INSTALLED && !installedApps.isEmpty()) {
            selectApp(
                    installedApps.get(0).getApplicationInfo().packageName,
                    installedApps.get(0).getLabel().toString()
            );
    ...
    private View buildHero(Category category, List<LauncherActivityInfo> installedApps) {
        ...
        TextView title = text(
                TextUtilsOr(scene.title, category.title),
                29,
                Color.WHITE,
                true
        );
        ...
        TextView summary = text(
                TextUtilsOr(scene.summary, category.subtitle),
                12,
        ...
        copyParams.rightMargin = dp(175);
        hero.addView(copy, copyParams);

        FrameLayout posterBadge = new FrameLayout(this);
        posterBadge.setBackground(posterBadgeBackground(scene.accentColor));
        posterBadge.setClipToOutline(true);

        ImageView posterImage = new ImageView(this);
        posterImage.setScaleType(ImageView.ScaleType.CENTER_CROP);
        posterImage.setContentDescription(TextUtilsOr(scene.posterTitle, category.title));
        posterBadge.addView(posterImage, new FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.MATCH_PARENT
        ));

        LinearLayout posterCaption = new LinearLayout(this);
        posterCaption.setOrientation(LinearLayout.VERTICAL);
        posterCaption.setGravity(Gravity.BOTTOM);
        posterCaption.setPadding(dp(14), dp(28), dp(14), dp(13));
        posterCaption.setBackground(posterCaptionBackground());
        TextView trending = text(getString(R.string.poster_now), 9,
                Color.rgb(197, 229, 255), true);
        trending.setLetterSpacing(0.15f);
        posterCaption.addView(trending);
        TextView badgeTitle = text(
                TextUtilsOr(scene.posterTitle, category.title),
                16,
                Color.WHITE,
                true
        );
        badgeTitle.setMaxLines(2);
        LinearLayout.LayoutParams badgeTitleParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
        );
        badgeTitleParams.topMargin = dp(4);
        posterCaption.addView(badgeTitle, badgeTitleParams);
        FrameLayout.LayoutParams captionParams = new FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.WRAP_CONTENT
        );
        captionParams.gravity = Gravity.BOTTOM;
        posterBadge.addView(posterCaption, captionParams);

        String expectedCategory = category.key;
        posterRepository.loadPoster(scene, bitmap -> {
            if (expectedCategory.equals(selectedCategory) && bitmap != null) {
        ...
        posterParams.gravity = Gravity.END | Gravity.CENTER_VERTICAL;
        hero.addView(posterBadge, posterParams);
        return hero;
    }
    ...
    private void addInstalledCard(LinearLayout row, LauncherActivityInfo info) {
    ...
    private View buildSettingsShelf() {
        ...
        if (!posterRepository.attributionName().isEmpty()) {
            TvCardView attribution = new TvCardView(
                    this,
                    ContextCompat.getDrawable(this, R.drawable.brand_tmdb),
                    getString(R.string.poster_data_source),
                    posterRepository.attributionNotice(),
                    posterRepository.attributionName(),
                    Color.rgb(1, 180, 228),
                    Color.rgb(13, 37, 63)
            );
            attribution.setOnClickListener(view -> {
                if (!posterRepository.attributionUrl().isEmpty()) {
                    BrowserActivity.open(this, posterRepository.attributionUrl());
                }
            });
            addCard(row, attribution, dp(242), dp(125));
        }
    ...
    private void selectApp(String packageName, String label) {
        ...
        if (heroPrimaryButton != null) {
            ...
            heroPrimaryButton.setText(getString(
                    heroPrimaryLabel(current, selectedCatalogApp(current))
            ));
        }
    }
    ...
    private int heroPrimaryLabel(Category category, CatalogApp app) {
        if (category.kind == CategoryKind.SETTINGS) {
            return R.string.environment_title;
        }
        return R.string.manage_apps;
    }
    ...
    private List<Category> categories() {
        ...
        result.add(new Category(
                ...
                CatalogAvailability.filter(AppCatalog.tvEntertainment())
        ));
        result.add(new Category(
                "china",
    ...
    private List<LauncherActivityInfo> launchableApps() {
        ...
        result.sort(Comparator.comparing(
                info -> info.getLabel().toString().toLowerCase(Locale.ROOT)
        ));
        return result;
    }
    ...
    private GradientDrawable posterBadgeBackground(int accent) {
        GradientDrawable value = new GradientDrawable(
                GradientDrawable.Orientation.TL_BR,
                new int[]{
                        Color.argb(120, Color.red(accent), Color.green(accent), Color.blue(accent)),
                        Color.argb(188, 6, 9, 18)
                }
        );
        value.setCornerRadius(dp(22));
        value.setStroke(dp(1), Color.argb(75, 255, 255, 255));
        return value;
    }

    private GradientDrawable posterCaptionBackground() {
        return new GradientDrawable(
                GradientDrawable.Orientation.TOP_BOTTOM,
                new int[]{
                        Color.TRANSPARENT,
                        Color.argb(178, 3, 6, 13),
                        Color.argb(238, 3, 6, 13)
                }
        );
    }

    private GradientDrawable brandMarkBackground() {
        GradientDrawable value = new GradientDrawable(
                GradientDrawable.Orientation.TL_BR,
                new int[]{
                        Color.rgb(95, 219, 255),
                        Color.rgb(91, 105, 255),
                        Color.rgb(152, 75, 255)
                }
        );
        value.setCornerRadius(dp(13));
        value.setStroke(dp(1), Color.argb(145, 255, 255, 255));
        return value;
    }
