...
public final class AptellyUpdateClient {
    ...
    private static final String GITHUB_LATEST_RELEASE =
            "https://api.github.com/repos/AptellyTV/aptellyTV/releases/latest";
    ...
    private static String releaseManifestUrl(JSONObject release) throws Exception {
        JSONArray assets = release.getJSONArray("assets");
        for (int index = 0; index < assets.length(); index++) {
            JSONObject asset = assets.getJSONObject(index);
            String name = asset.optString("name", "");
            String url = asset.optString("browser_download_url", "");
            if (name.matches("^Aptelly-[0-9]+\\.[0-9]+\\.[0-9]+-release\\.json$")
                    && url.startsWith(
                    "https://github.com/AptellyTV/aptellyTV/releases/download/"
            )) {
                return url;
            }
        }
    ...
    private static HttpURLConnection open(String address) throws Exception {
        ...
        for (int redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
            ...
            connection.setRequestProperty("User-Agent", "Aptelly-Android-TV");
            int code = connection.getResponseCode();
    ...
    static boolean isRedirectAllowed(URL current, URL next) {
        ...
        return ("github.com".equals(currentHost)
                || "api.github.com".equals(currentHost)
                || isGitHubReleaseAssetHost(currentHost))
